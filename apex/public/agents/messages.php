    ...
    <table>
      ...
      <?php while($message = db_fetch_assoc($message_result)) { ?>
        <?php
          ...
          $senderTemp = $message['sender_id'];
          $senderTemp = find_agent_by_id($senderTemp);
          $sender = db_fetch_assoc($senderTemp);
          $encrypted_text = $message['cipher_text'];
          $private_key = $current_user['private_key'];
          $decrypted_text = pkey_decrypt($encrypted_text, $private_key);
          $message_text = $decrypted_text;
          $validity_text = $message['signature'];
          // Oooops.
